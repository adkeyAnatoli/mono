...
const LastSection = () => {
  ...
  const contentRaw = t.raw('content');
  const tablesRaw = t.raw('tables');
  const data = {
    title: t('title'),
    content: (Array.isArray(contentRaw) ? contentRaw : []) as ContentItem[],
    tables: (Array.isArray(tablesRaw) ? tablesRaw : []) as LastSectionTable[],
  };
