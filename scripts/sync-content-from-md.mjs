...
function appTabContentBeforeSecondH3(nodes) {
  const result = [];
  let h3Count = 0;
...
function extractH3SubsectionNodes(nodes, headingAliases) {
  const targets = new Set(headingAliases.map(normalizeHeading));
  for (let i = 0; i < nodes.length; i++) {
    const n = nodes[i];
    if (n.type !== 'heading' || n.depth !== 3) continue;
    const key = normalizeHeading(headingPlainText(n));
...
function main() {
  ...
  const appPrimaryNodes = appTabContentBeforeSecondH3(mobile);
  const mobileHeading = by.app?.title || 'Mobile Gaming';

  const langsFromAppH3 = extractH3SubsectionNodes(mobile, [
    'available languages',
    ...aliases.languages,
  ]);
  const langsNodes =
  ...
  const cursFromAppH3 = extractH3SubsectionNodes(mobile, [
  ...
  const langsHeading = 'Available Languages';
  const cursHeading = 'Supported Currencies';
