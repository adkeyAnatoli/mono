...
const CONTENT_MD = path.join(PROJECT, 'main.md');
...
const PAGE_FILES = {
  ...
  rewards: 'rewards.md',
  resorts: 'resorts.md',
  'first-day-tips': 'first-day-tips.md',
  faq: 'faq.md',
  terms: 'terms.md',
  privacy: 'privacy.md',
};
...
function listItemText(body, li) {
  ...
  if (raw === null) {
    return normalizeMdText(li.children.map((c) => serializeNode(c)).join('\n'));
  }
...
function buildFaq(nodes, body, sectionTitle = 'FAQ') {
  ...
  for (const b of blocks) {
    ...
    if (isFaqSection(b.heading)) continue;
  ...
  return { title: sectionTitle, faqList };
}
...
function titleBeforeColon(heading) {
  const t = plainContentText(heading);
  const idx = t.indexOf(':');
  return idx === -1 ? t : t.slice(0, idx).trim();
}

function splitNestedFaq(nodes) {
  const faqIndex = nodes.findIndex(
    (n) =>
      n.type === 'heading' && n.depth === 3 && isFaqSection(headingPlainText(n))
  );
  if (faqIndex === -1) return { before: nodes, faqNodes: null };
  return {
    before: nodes.slice(0, faqIndex),
    faqNodes: nodes.slice(faqIndex + 1),
  };
}
...
function buildPageJson(raw, slug) {
  ...
  for (const section of pageSections) {
    ...
    const nested = splitNestedFaq(section.nodes);
    if (nested.faqNodes) {
      faq = buildFaqFromNodes(nested.faqNodes, body);
    }
    contentSections.push({
      heading: plainContentText(section.title),
      content: nodesToContentItems(nested.before, body),
    });
  }
...
function main() {
  ...
  const introTitle = titleBeforeColon(h1Title);

  writeJson('dataCasinoIntro.json', {
    title: introTitle,
    image: {
      src: '/sectionImg/slot3.webp',
      alt: introTitle + ' illustration',
    },
    highlights: [],
    content: nodesToContentItems(introNodes, body),
    whoWeAre: {
  ...
  writeJson('dataLast.json', {
    title: 'Resorts vs Online',
    content: nodesToContentItems(by.get('Resorts vs Online') || [], body),
  });

  writeJson(
    'dataCasinoInfo.json',
    buildSectionWithH3Blocks(
      by.get('Who Each Format Actually Suits') || [],
      body,
  ...
  writeJson('dataSupport.json', {
    title: 'What Actually Sets the Two Formats Apart',
    content: nodesToContentItems(
      by.get('What Actually Sets the Two Formats Apart') || [],
      body
    ),
  });
  ...
  writeJson('dataFaq.json', buildFaq(by.get(faqTitle) || [], body, faqTitle));
