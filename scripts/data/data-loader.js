...
export async function loadData() {
  if (!SITE_ID || SITE_ID === "YOUR_SITE_ID_HERE") {
    console.warn("SITE_ID is not set. Using mock data.");
    setupMockData();
    return;
  }
  ...
  try {
    ...
    try {
      appState.payments = await fetchJson(
        `https://api.adkey-seo.com/api/website/get-payments/${SITE_ID}`
      );
    } catch (e) {
    ...
    try {
      appState.providers = await fetchJson(
